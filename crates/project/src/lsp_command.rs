...
pub(crate) struct SemanticTokensFullResponse {
    ...
    // If this is None, the semantic tokens are useless. This is only an option because
...
impl LspCommand for SemanticTokensRange {
    type Response = SemanticTokensFullResponse;
    type LspRequest = lsp_semantic_tokens::SemanticTokensRangeRequest;
    type ProtoRequest = proto::GetSemanticTokensRange;
    ...
    fn to_lsp(
        ...
        let range_start = self.range.start.to_point_utf16(&buffer_snapshot);
        let range_end = self.range.end.to_point_utf16(&buffer_snapshot);
        ...
        Ok(lsp::SemanticTokensRangeParams {
            ...
            range: lsp::Range::new(
                point_to_lsp(range_start),
                point_to_lsp(range_end),
            ),
            partial_result_params: Default::default(),
            work_done_progress_params: Default::default(),
        })
    ...
    async fn response_from_lsp(
        ...
        match message {
            ...
            Some(lsp_semantic_tokens::SemanticTokensFullResult::Partial(_)) => {
                anyhow::bail!(
                    "Unexpected semantic tokens response with partial result for range request"
                )
            }
            None => Ok(Default::default()),
        }
    ...
    fn to_proto(&self, project_id: u64, buffer: &Buffer) -> proto::GetSemanticTokensRange {
        proto::GetSemanticTokensRange {
            project_id,
            buffer_id: buffer.remote_id().into(),
            version: serialize_version(&buffer.version()),
    ...
    async fn from_proto(
        message: proto::GetSemanticTokensRange,
        _: Entity<LspStore>,
        buffer: Entity<Buffer>,
        mut cx: AsyncApp,
    ) -> Result<Self> {
        ...
        let range_start = message.start.ok_or_else(|| anyhow::anyhow!("Missing range start"))?;
        let range_end = message.end.ok_or_else(|| anyhow::anyhow!("Missing range end"))?;

        let start_anchor = deserialize_anchor(range_start)
            .ok_or_else(|| anyhow::anyhow!("Invalid range start anchor"))?;
        let end_anchor = deserialize_anchor(range_end)
            .ok_or_else(|| anyhow::anyhow!("Invalid range end anchor"))?;
    ...
    fn buffer_id_from_proto(message: &proto::GetSemanticTokensRange) -> Result<BufferId> {
        BufferId::new(message.buffer_id)
    }
}
    ...
    fn response_to_proto(
        ...
        match response {
            SemanticTokensDeltaResponse::Full {
                ...
                edits: vec![],
                result_id: id,
                server_id: server_id.map(|id| id.to_proto()).unwrap_or_default(),
                version: serialize_version(buffer_version),
            },
            SemanticTokensDeltaResponse::Delta {
                edits,
    ...
    async fn response_from_proto(
        ...
        if !message.data.is_empty() {
            Ok(SemanticTokensDeltaResponse::Full {
                data: message.data,
                id: message.result_id,
                server_id: Some(LanguageServerId::from_proto(message.server_id)),
            })
        } else {
            Ok(SemanticTokensDeltaResponse::Delta {
                edits: message
...
mod lsp_semantic_tokens {
    ...
    impl lsp::request::Request for SemanticTokensRangeRequest {
