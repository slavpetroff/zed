...
impl MultiBufferSnapshot {
    ...
    pub fn excerpts_for_range<T: ToOffset>(
        ...
        std::iter::from_fn(move || {
            ...
            let buffer_offset = BufferOffset(excerpt.range.context.start.to_offset(&excerpt.buffer));
            let excerpt_offset = *cursor.excerpts.start();
            let diff_transforms = cursor.diff_transforms.clone();
            cursor.next_excerpt();
...
impl<'a> MultiBufferExcerpt<'a> {
    ...
    pub fn contains_partial_buffer_range(&self, range: Range<usize>) -> bool {
        range.start <= self.excerpt.buffer_end_offset().0
            && range.end >= self.excerpt.buffer_start_offset().0
    }
