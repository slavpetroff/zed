...
pub(crate) const MINIMAP_FONT_SIZE: AbsoluteLength = AbsoluteLength::Pixels(px(2.0));
...
impl Editor {
    ...
    pub fn new_file(
        ...
        Self::new_in_workspace(workspace, window, cx).detach_and_prompt_err(
            ...
            |e, _, _| {
                match e.error_code() {
                    ErrorCode::RemoteUpgradeRequired =>
                        Some(
                            format!(
                    ...
                    .map(|selection| {
                        ...
                        let (
                            ...
                            let comment_delimiter = maybe!({
                                ...
                                let cursor_is_placed_after_comment_marker = num_of_whitespaces
                                    + trimmed_len
    ...
    pub fn visible_excerpts(
        ...
        let multi_buffer_visible_end = multi_buffer_snapshot.clip_point(
            multi_buffer_visible_start
                + Point::new(self.visible_line_count().unwrap_or(0.0).ceil() as u32, 0),
            Bias::Left,
        );
    ...
    const EDIT_PREDICTION_POPOVER_PADDING_X: Pixels = px(24.0);
    const EDIT_PREDICTION_POPOVER_PADDING_Y: Pixels = px(2.0);
    ...
    fn render_edit_prediction_popover(
        ...
        match &active_edit_prediction.completion {
            ...
            EditPrediction::MoveOutside { snapshot, .. } => {
                ...
                let origin_x = text_bounds.size.width / 2.0 - size.width / 2.0;
                let origin_y = text_bounds.size.height - size.height - px(30.0);
    ...
    fn render_edit_prediction_modifier_jump_popover(
        ...
        const SCROLL_PADDING_Y: Pixels = px(12.0);
        ...
        const POLE_WIDTH: Pixels = px(2.0);
        ...
        let target_y =
            target_display_point.row().as_f64() * f64::from(line_height) - scroll_pixel_position.y;

        let flag_on_right = target_x < text_bounds.size.width / 2.0;
    ...
    fn render_edit_prediction_eager_jump_popover(
        ...
        if target_display_point.row().as_f64() < scroll_top {
            ...
            let size = element.layout_as_root(AvailableSpace::min_size(), window, cx);
            let offset = point(
                (text_bounds.size.width - size.width) / 2.0,
            ...
            element.prepaint_at(origin, window, cx);
            Some((element, origin))
        } else if target_display_point.row().as_f64() + 1.0 > scroll_bottom {
            let mut element = self
                .render_edit_prediction_line_popover(
                    "Jump to Edit",
    ...
    fn render_edit_prediction_end_of_line_popover(
        ...
        if origin.x > max_x {
            ...
            origin.x = content_origin.x + editor_width - size.width - px(2.0);
        }
    ...
    fn render_edit_prediction_diff_popover(
        ...
        let mut element = h_flex()
            ...
            .child(
                h_flex()
                    .h(line_height + BORDER_WIDTH * 2.0)
                    .px_1p5()
                    .gap_1()
    ...
    fn render_edit_prediction_line_popover(
        ...
        let padding_right = if icon.is_some() { px(4.0) } else { px(8.0) };
    ...
    fn refresh_runnables(&mut self, window: &mut Window, cx: &mut Context<Self>) -> Task<()> {
        ...
        cx.spawn_in(window, async move |editor, cx| {
            ...
            let new_rows = cx.background_spawn({
